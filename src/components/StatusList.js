...
const StatusList = () => {
    ...
    const handleDelete = () => {
        axios.delete(`http://localhost:8000/produk/api/status/${deleteId}/`)
            .then(() => {
    ...
    return (
        <div className="max-w-7xl mx-auto p-6 bg-white shadow-lg rounded-lg mt-10">
            <h2 className="text-2xl font-bold mb-6">Daftar Status</h2>
            <Link
                to="/status-tambah"
            ...
            <table className="min-w-full table-auto border-collapse">
                <thead className="bg-blue-600 text-white">
                    <tr>
                        <th className="px-6 py-3 text-left">Nama Status</th>
                ...
                <tbody>
                    {categories.map(category => (
                        <tr key={category.id} className="border-b hover:bg-gray-100">
                            <td className="px-6 py-4">{category.nama_status}</td>
                            <td className="px-6 py-4">
                                <Link to={`/status-edit/${category.id}`} className="text-blue-600 hover:underline mr-4">
                                    Edit
                                </Link>
                                <button
